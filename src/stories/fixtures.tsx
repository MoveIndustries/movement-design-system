// Shared synthetic fixtures for Storybook previews only (not shipped). These
// stand in for icons that arrive at runtime from the wallet adapters — e.g. the
// keyless adapter registers its own Google mark, so the DS ships no Google
// icon. Kept in one place so stories don't each re-hand-roll the same SVG.

/** Inline Google "G" mark as a data URI (matches the keyless adapter's icon). */
export const GOOGLE_ICON_DATA_URI =
