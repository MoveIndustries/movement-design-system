...
function Footer({
  ...
  return (
    <footer
      data-slot="footer"
      className={cn("mt-8 w-full bg-black text-white md:mt-12", className)}
      {...props}
    >
      <div className="mx-auto max-w-[var(--container-max,1728px)] px-[var(--container-padding-x,clamp(1rem,5.8vw,100px))] py-14 md:py-20">
        ...
        <div className="flex flex-col gap-10 md:flex-row md:items-start md:justify-between md:gap-16">
          ...
          >
            <MovementMonogram />
            <span className="text-[24px] font-normal leading-none tracking-[-0.042em] [font-family:var(--font-display)] md:text-[36px]">
              {brandLabel}
            </span>
          </a>

          {/* Link columns */}
          <div className="flex gap-10 sm:gap-14 md:gap-12 lg:gap-16">
            {columns.map((column) => (
        ...
        {/* Divider */}
        <div className="mt-12 h-px w-full bg-white/12 md:mt-16" />

        {/* Legal row */}
        <div className="mt-6 flex flex-col-reverse items-start gap-4 md:mt-8 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[12px] [font-family:var(--font-display)]">
            <span className="font-medium text-white">{copyrightText}</span>
            {legalLinks.map((link) => (
              <FooterAnchor
                key={link.href}
                link={link}
...
function FooterColumnComponent({ column }: { column: FooterColumn }) {
  return (
    <div>
      <h2 className="mb-2 text-[14px] font-bold leading-[1.35] text-white [font-family:var(--font-display)]">
        {column.title}
      </h2>
