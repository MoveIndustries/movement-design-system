...
- **TransactionApprovalModal — dismissing while `loading` no longer reports a rejection.** Escape, overlay click and mobile swipe closed the modal mid-signing and fired `onReject()` for a transaction that may already have been submitted. The modal is now non-dismissable while `loading`, matching the already-disabled buttons.
- **TransactionApprovalModal — an undecodable amount blocks approval.** A recognized transfer whose amount can't be read rendered a bare em-dash next to a live Approve button, which reads as "zero" or "not applicable". Approve is now disabled with an explicit warning; decoding moved into a pure `decodePayload` so the modal can gate on it.
- **TransactionApprovalModal — `0x1::coin::transfer` decodes as a coin transfer.** It shares the `transfer_coins` signature but previously fell through to the generic "Call function" view.
- **TransactionApprovalModal — fee-payer role is now a callout, not a footnote clause.** Paying gas for someone else's transaction is the highest-consequence fact on the screen; it no longer renders at the same weight as "Sign and submit". A payload-level `feePayer` (a separate sponsor) and `asFeePayer` (this signer pays) are worded distinctly rather than merged.
